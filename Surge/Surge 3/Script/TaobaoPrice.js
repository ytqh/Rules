        ...
        if (dns && dns.length > 0) {
            ...
            while (i--) {
                ...
                if (element.host == host) {
                    element.ips = [] if (consoleLog) console.log(JSON.stringify(element))
                }
            }
        }
    ...
    requestPrice(shareUrl, function(data) {
        if (data) {
            if (obj.data.apiStack) {
                let apiStack = obj.data.apiStack[0] let value = JSON.parse(apiStack.value)
                ...
                if (trade && trade.useWap == "true") {
                    ...
                    if (vertical && vertical.hasOwnProperty("tmallhkDirectSale")) {
                        ...
                        //     value.global.data.tradeConsumerProtection = setTradeConsumerProtection(data,
                        //     value.global.data.tradeConsumerProtection)
                        // } else {
                        //     value["tradeConsumerProtection"] = customTradeConsumerProtection()
                        //     value.tradeConsumerProtection = setTradeConsumerProtection(data,
...
function sendNotify(data, shareUrl) {
    if (data.ok == 1 && data.single) {
        const lower = lowerMsgs(data.single)[0] const detail = priceSummary(data)[1] const tip =
            data.PriceRemark.Tip + "（仅供参考）"
        $tool.notify("", "",
                     `〽️历史${lower} ${tip}\n${
...
function setConsumerProtection(data, consumerProtection) {
    ...
    if (data.ok == 1 && data.single) {
        ...
        const summary = priceSummary(data)[1] const item =
            customItem(lower[1], [ `${lower[0]} ${tip}（仅供参考）\n${summary}` ])
...
function setTradeConsumerProtection(data, tradeConsumerProtection) {
    ...
    if (data.ok == 1 && data.single) {
        const lower = lowerMsgs(data.single)
        const tip = data.PriceRemark.Tip
        const tbitems = priceSummary(data)[0] const item = customItem(lower[1], `${lower[0]} ${tip}（仅供参考）`)
...
function customItem(title, desc) {
    return {
        icon: "https://s2.ax1x.com/2020/02/16/3STeIJ.png", title: title, name: title, desc: desc
    }
}
...
function customTradeConsumerProtection() {
    return {
        "tradeConsumerService":
            {"service": {"items": [], "icon": "", "title": "基础服务"}, "nonService": {"items": [], "title": "其他"}},
            "passValue": "all", "url": "https://h5.m.taobao.com/app/detailsubpage/consumer/index.js", "type": "0"
    }
}
...
function
    Tool() {
        ...
        _isQuanX =
            typeof $task != "undefined" this.isSurge = _isSurge this.isQuanX = _isQuanX this.isResponse =
                typeof $response != "undefined" this.notify = (title, subtitle, message) => {
                    if (_isQuanX)
                    ...
                    if (_isQuanX) {
                        if (typeof options == "string")
                            options = {url : options} options["method"] = "GET"
                            $task.fetch(options).then(response => {callback(null, _status(response), response.body)},
                                                      reason => callback(reason.error, null, null))
                    }
                    if (_isSurge)
                        $httpClient.get(options, (error, response, body) => {callback(error, _status(response), body)})
                        if (_node) _node.request(options,
                                                 (error, response, body) => {callback(error, _status(response), body)})
                } this.post = (options, callback) => {
                    if (_isQuanX) {
                        if (typeof options == "string")
                            options = {url : options} options["method"] = "POST"
                            $task.fetch(options).then(response => {callback(null, _status(response), response.body)},
                                                      reason => callback(reason.error, null, null))
                    }
                    if (_isSurge)
                        $httpClient.post(options, (error, response, body) => {callback(error, _status(response), body)})
                        if (_node) _node.request.post(
                            options, (error, response, body) => {callback(error, _status(response), body)})
                } _status = (response) => {
                    if (response) {
                        if (response.status) {
